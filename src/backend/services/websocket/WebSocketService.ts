...
import { bondingCurveService } from '../blockchain/BondingCurveService';

export class WebSocketService {
  private io: SocketIOServer | null = null;
  private priceUpdateInterval: NodeJS.Timeout | null = null;
  private connectedClients: Map<string, Set<string>> = new Map(); // tokenAddress -> socketIds
  ...
  initialize(server: HTTPServer) {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('subscribe', async (tokenAddress: string) => {
   ...
   */
  broadcastGraduation(tokenAddress: string, graduationData: any) {
    if (!this.io) return;
  ...
  getStats() {
    const stats = {
      ...
      tokenSubscriptions: {} as Record<string, number>
    };
