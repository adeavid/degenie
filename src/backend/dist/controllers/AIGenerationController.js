...
const AssetGenerationService_1 = require("../services/ai/AssetGenerationService");
const CreditService_1 = require("../services/CreditService");
const aiValidators_1 = require("../validators/aiValidators");
const userUtils_1 = require("../utils/userUtils");
class AIGenerationController {
    assetService;
    creditService;
    constructor() {
        this.assetService = new AssetGenerationService_1.AssetGenerationService();
    ...
    async generateAsset(req, res) {
        try {
            ...
            // Validate request
            const validation = (0, aiValidators_1.validateGenerationRequest)(req.body);
            if (!validation.valid) {
                res.status(400).json({ error: validation.error });
                return;
            }
            const { prompt, assetType, tokenSymbol } = req.body;
            // Get user tier
            const userTier = await (0, userUtils_1.getUserTier)(userId);
            ...
            const result = await this.assetService.generateAsset(userId, prompt, assetType, userTier, tokenSymbol);
    ...
    async batchGenerate(req, res) {
        try {
            ...
            for (const request of requests) {
                try {
                    const result = await this.assetService.generateAsset(userId, request.prompt, request.assetType, userTier, request.tokenSymbol);
    ...
    getRequiredCredits(assetType, tier) {
        const costs = this.creditService.getCreditCosts();
        const costMap = {
...
//# sourceMappingURL=AIGenerationController.js.map