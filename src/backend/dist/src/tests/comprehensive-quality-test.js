...
const quality_report_generator_1 = require("./quality-report-generator");
async function comprehensiveQualityTest() {
    ...
    // Create test user
    try {
        await axios_1.default.post(`${baseUrl}/api/users`, {
    ...
    for (const test of testCases) {
        try {
            ...
            const response = await axios_1.default.post(`${baseUrl}/api/generate/${test.type}`, {
                prompt: test.prompt,
                tier: test.tier,
                userId: testUserId
            });
    ...
    await quality_report_generator_1.QualityReportGenerator.saveHTMLReport(reportData);
    // Print summary
    console.log('\n' + '='.repeat(50));
...
function evaluateQuality(data, test) {
    ...
    const keywordMatches = test.expectedKeywords.filter((kw) => optimizedPrompt.includes(kw.toLowerCase()));
...
function generateSummary(results) {
    ...
    validResults.forEach(r => {
        const tierArray = tierScores[r.tier];
        if (tierArray) {
...
//# sourceMappingURL=comprehensive-quality-test.js.map