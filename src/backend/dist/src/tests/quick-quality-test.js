...
async function quickQualityTest() {
    ...
    for (const test of tests) {
        try {
            ...
            const response = await axios_1.default.post(`${baseUrl}/api/generate/${test.type}`, { ...test, userId: testUserId });
...
function calculateSimpleScore(quality, tier) {
    let score = 50; // Base score
    // Resolution scoring
    if (quality.width >= 1024)
        score += 20;
    else if (quality.width >= 512)
        score += 10;
    // Steps scoring
    if (quality.steps >= 50)
        score += 20;
    else if (quality.steps >= 25)
        score += 15;
    else if (quality.steps >= 15)
        score += 10;
    // Tier bonus
    if (tier === 'viral')
        score += 10;
...
//# sourceMappingURL=quick-quality-test.js.map