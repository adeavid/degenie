...
exports.blockchainIndexer = exports.BlockchainIndexer = void 0;
...
// Program IDs
const PUMP_FUN_PROGRAM_ID = new web3_js_1.PublicKey('6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P'); // pump.fun mainnet
const DEGENIE_PROGRAM_ID = new web3_js_1.PublicKey('Cu3ZCXsVh7xC64gWH23vjDeytWC6ZGcMRVYZAka92QTq'); // Our devnet program
class BlockchainIndexer {
    ...
    subscribeToLogs() {
        ...
        this.connection.onLogs(DEGENIE_PROGRAM_ID, async (logs, ctx) => {
    ...
    async pollHistoricalTrades() {
        while (this.isRunning) {
            try {
                // Get recent signatures for our program
                const signatures = await this.connection.getSignaturesForAddress(DEGENIE_PROGRAM_ID, { limit: 100 }, 'confirmed');
    ...
    async processTokenCreation(tx) {
        try {
            ...
            WebSocketService_1.webSocketService.broadcastTokenCreated({
                address: tokenAddress,
                name,
                symbol,
                creator,
    ...
    async processTrade(tx, type) {
        try {
            ...
            newPrice = (initialPrice * Math.pow(1 + k, newTotalSupply / 1e6)) / web3_js_1.LAMPORTS_PER_SOL;
            ...
            // Update candles
            await CandleService_1.candleService.updateCandlesFromTrade(trade);
            // Broadcast via WebSocket
    ...
    async processCandlesPeriodically() {
        while (this.isRunning) {
            try {
                await CandleService_1.candleService.processAllCandles();
            }
            catch (error) {
...
exports.blockchainIndexer = new BlockchainIndexer();
//# sourceMappingURL=BlockchainIndexer.js.map